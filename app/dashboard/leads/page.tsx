...
import { addCompanyToCrmAction, sendListToCrmAction } from "@/app/dashboard/crm/actions";
import { dealIdsByEstablishment } from "@/lib/crm/repository";
...
export default async function LeadsPage({ searchParams }: LeadsPageProps) {
  ...
  // CRM (Fase 6): quais empresas já têm negócio no workspace ativo. Opcional —
  // sem a migração do CRM a carteira continua funcionando.
  const crmDealByEstablishment = await getCrmSession()
    .then((session) => (session ? dealIdsByEstablishment(session.ctx, leads.map((lead) => lead.establishmentId)) : new Map<string, string>()))
    .catch(() => null);
  const crmFeedback = readCrmFeedback(params);
  const statusMessage = readStatusMessage(status) || crmFeedback.status;
  const errorMessage = readErrorMessage(error) || crmFeedback.error;
