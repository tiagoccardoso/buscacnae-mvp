...
export async function POST(request: Request) {
  try {
    ...
    if (searchesError || !searches || searches.length === 0 || searches.length !== selectedSearchIds.length) {
    ...
    console.error("[history-bulk-checkout] failed to create checkout session", error);
    return NextResponse.redirect(new URL("/dashboard/history?error=falha-checkout-multiplo", getBaseUrl()), 303);
  }
}
