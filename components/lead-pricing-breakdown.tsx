    ...
    <div className="stack" style={{ gap: 12 }}>
      <div className="grid-2">
        {summary.tiers.map((tier) => (
          <div key={tier.key} className="surface-soft card stack" style={{ gap: 6 }}>
            <span className="kicker">{tier.label}</span>
            <strong style={{ fontSize: "1.6rem" }}>{tier.count}</strong>
            <span className="muted">{formatMoney(tier.unitAmountCents / 100)} por lead · subtotal de {formatMoney(tier.subtotalAmountCents / 100)}</span>
      ...
      <div className="notice">
        ...
        {hasAdjustment ? (
          <> Há um ajuste de valor mínimo operacional aplicado ao checkout desta lista.</>
        ) : null}
      </div>
    </div>
  );
